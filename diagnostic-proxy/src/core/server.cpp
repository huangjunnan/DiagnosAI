...
LocalProxyServer::LocalProxyServer(std::shared_ptr<ToolRegistry> registry) : registry_(std::move(registry))
{
}
...
void LocalProxyServer::start(int port)
{
    ...
    svr.Post(
        ...
        [this](const httplib::Request& req, httplib::Response& res)
        {
            ...
            try
            {
                ...
                L_DEBUG("工具 {} 执行完成，耗时 {} ms，原始输出大小 {} 字节", toolName, exec_ms, raw.size());
    ...
    std::cout << "Diagnostic proxy listening on 0.0.0.0:" << port << '\n';
