...
void conditional_early_return_leak(bool need_early_return) {
  std::cout << "[conditional_early_return_leak] 进入函数，分配 50MB ..."
            << '\n';
  int* data = new int[50 * 1024 * 1024 / sizeof(int)];  // 约 50 MB
  if (need_early_return) {
    std::cout << "[conditional_early_return_leak] 提前 return ! 内存未被释放 !"
              << '\n';
    return;  // ❌ 这里直接返回，data 泄漏
  }

  // 正常路径
  std::cout << "[conditional_early_return_leak] 正常路径，释放内存。" << '\n';
  delete[] data;
}
...
// ========== 主函数 ==========
int main() {
  std::cout << "复杂内存泄漏测试程序启动，PID: " << getpid() << '\n';

  // --- 演示提前 return 泄漏 ---
  std::cout << "\n>>> 第一次调用 conditional_early_return_leak(true)，触发提前 "
               "return 泄漏"
            << '\n';
  conditional_early_return_leak(true);
  std::this_thread::sleep_for(std::chrono::seconds(1));

  std::cout << "\n>>> 第二次调用 conditional_early_return_leak(false)，正常执行"
            << '\n';
  conditional_early_return_leak(false);

  // --- 演示异常泄漏 ---
  std::cout << "\n>>> 调用 exception_leak()，将抛出异常导致泄漏" << '\n';
  try {
    exception_leak();
